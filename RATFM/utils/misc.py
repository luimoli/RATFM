...
def get_patch_all(fine_arr, n): #图的区域是n*n
    '''
...
def recover_patch_from_arr(arr_patches, n):
    ...
    '''
    arr = arr_patches
    split_size = arr.shape[0] // (n*n) #一张图切成n*n块,dim0= batch_size*n*n, split_size=batch_size
    fine_patch_list = torch.split(arr, split_size, dim=0) # [batch,2,16,16]
    row_list=[]
    for i in range(0,n*n,n):
        row_list.append(torch.cat(fine_patch_list[i:i+n], 3)) #[batch,2,16,64] get all the rows.
    finemap = torch.cat(row_list, 2) #[batch,2,64,64]
    return finemap
